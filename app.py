...
# ─────────────────────────────────────────────
# GLOBAL STYLE (Light terminal aesthetic)
# ─────────────────────────────────────────────
st.markdown("""
<style>
  /* ── Base ── */
...
def build_chart(df: pd.DataFrame, ticker: str, n_candles: int = 100) -> go.Figure:
    ...
    # 1. Candlestick (Standard High-Visibility Green/Red)
    fig.add_trace(
        go.Candlestick(
    ...
    fig.update_layout(
        ...
        hoverlabel=dict(
            ...
            font_size=11, font_family="JetBrains Mono, monospace", font_color="#0f172a"
        ),
...
with tab_backtest:
    if trades_df.empty or not bt_stats:
        ...
        def _style_trades(row):
            if row["Result"] == "Win":
                return ["background-color:#dcfce7; color:#16a34a"] * len(row)
            if row["Result"] == "Loss":
